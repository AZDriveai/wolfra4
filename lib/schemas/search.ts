...
export const searchSchema = z.object({
  ...
  max_results: z.number().optional().describe("The maximum number of results to return. default is 20"),
  search_depth: z.string().optional().describe('The depth of the search. Allowed values are "basic" or "advanced"'),
  include_domains: z
    .array(z.string())
    .optional()
    .describe(
...
export const strictSearchSchema = z.object({
  ...
  search_depth: z.enum(["basic", "advanced"]).describe("The depth of the search"),
  include_domains: z
    .array(z.string())
    .describe(
      "A list of domains to specifically include in the search results. Default is None, which includes all domains.",
...
export function getSearchSchemaForModel(fullModel: string) {
  ...
  const useStrictSchema = (provider === "openai" || provider === "azure") && modelName?.startsWith("o")
